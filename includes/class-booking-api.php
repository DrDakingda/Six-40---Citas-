...
class Six40_Booking_API {
	...
	/**
	 * Get all services (base + additional).
	 *
	 * @return array|WP_Error
	 */
	 ...
	 */
	public function get_available_slots( $location, $date, $base_service_id ) {
		// Validate date
		...
		$day_of_week = (int) $dt_date->format( 'N' ) % 7; // 0=Mon, 6=Sun
	...
	public function create_appointment( $data ) {
		...
		$base_service_id = (int) ( $data['base_service_id'] ?? 0 );
		$additional_ids = $data['additional_service_ids'] ?? [];
		...
		// Validate service
		$service = $this->get_service( $base_service_id );
		if ( ! $service || $service['type'] !== 'base' ) {
			return new WP_Error( 'invalid_service', 'Invalid base service.' );
		}

		// Calculate total duration
		$service_ids = array_merge( [ $base_service_id ], (array) $additional_ids );
		$total_duration = $this->calculate_service_duration( $service_ids );
	...
	private function find_free_barber( $location, $date, $start_time, $service_ids ) {
		...
		// Get day of week
		$dt_date = \DateTime::createFromFormat( 'Y-m-d', $date );
		$day_of_week = (int) $dt_date->format( 'N' ) % 7;

...
/**
 * WordPress Admin Panel for Six40 Booking System.
 */
class Six40_Admin_Panel {
    ...
    private function __construct() {
        ...
        add_action( 'admin_post_six40_update_barber',       [ $this, 'handle_update_barber' ] );
    ...
    public function page_dashboard(): void {
        ...
        if ( is_wp_error( $today_appts ) ) {
            $today_appts = [];
        }

        $services = Six40_Booking_API::SERVICES;
        $barbers  = Six40_Booking_API::BARBERS;
    ...
    public function page_citas(): void {
        ...
        if ( is_wp_error( $appointments ) ) {
            $appointments = [];
            $error        = $appointments->get_error_message();
        }

        // Filter by date_from client-side since Supabase PostgREST gte.
        if ( $date_from ) {
            $appointments = array_filter( $appointments, fn( $a ) => ( $a['date'] ?? '' ) >= $date_from );
        }
    ...
    public function handle_save_settings(): void {
        ...
        $fields = [
            'supabase_url', 'supabase_key', 'google_client_id', 'google_client_secret',
            'google_calendar_malaga', 'google_calendar_torremolinos',
            'resend_api_key', 'email_from', 'email_from_name',
        ];
        $cfg = (array) get_option( 'six40_settings', [] );
        foreach ( $fields as $field ) {
            $cfg[ $field ] = sanitize_text_field( $_POST[ $field ] ?? '' );
        }
    ...
    public function handle_oauth_callback(): void {
        if ( ! isset( $_GET['page'], $_GET['oauth'], $_GET['code'] ) ) {
            return;
        }
        if ( $_GET['page'] !== 'six40-settings' || $_GET['oauth'] !== 'google' ) {
            return;
        }
        if ( ! current_user_can( 'manage_options' ) ) {
            return;
        ...
        $msg      = is_wp_error( $result ) ? '&oauth_error=1' : '&oauth_success=1';
        wp_redirect( admin_url( 'admin.php?page=six40-settings' . $msg ) );
        exit;
    }
    ...
    public function ajax_update_appt_status(): void {
        ...
        if ( $status === 'cancelled' && ! empty( $result[0] ) ) {
            $mailer = new Six40_Email();
            $mailer->send_cancellation( $result[0] );
        }

        wp_send_json_success( [ 'status' => $status ] );
    }

    // ── AJAX: Update barber status ────────────────────────────────────────────
    ...
    public function ajax_update_barber_status(): void {
        ...
        $api    = new Six40_Booking_API();
        $result = $api->update_barber_status( $barber_id, $status );
    ...
    // ── AJAX: Get appointments as JSON (for calendar) ─────────────────────────
    ...
    public function ajax_get_appointments_json(): void {
        ...
        if ( is_wp_error( $appointments ) ) {
            wp_send_json_error( $appointments->get_error_message() );
        }
        ...
        foreach ( $appointments as $a ) {
            ...
            $events[] = [
                ...
                'start'           => sprintf( '%sT%s', $a['date'] ?? '', substr( $a['time'] ?? '', 0, 5 ) ),
                'end'             => sprintf( '%sT%s', $a['date'] ?? '', substr( $a['end_time'] ?? '', 0, 5 ) ),

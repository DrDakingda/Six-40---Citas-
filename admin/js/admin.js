...
(function ($) {
  ...
  $(document).on('change', '.six40-status-select', function () {
    ...
    $.post(six40Admin.ajaxUrl, {
      action:    'six40_update_appt_status',
      nonce:     six40Admin.nonce,
      id:        id,
      status:    status
    }, function (res) {
      if (res.success) {
        var labels = {
          ...
          cancelled: 'Cancelada',  no_show:   'No presentó'
        };
        $row.find('.six40-status')
          .attr('class', 'six40-status six40-status--' + status)
  ...
  $(document).on('focus', '.six40-status-select', function () {
  ...
  $(document).on('click', '.six40-barber-btn', function () {
    var $btn     = $(this);
    var barberId = $btn.data('id');
    var status   = $btn.data('status');
    var $card    = $btn.closest('.six40-barber-card');

    $btn.prop('disabled', true).text('…');

    $.post(six40Admin.ajaxUrl, {
      action:    'six40_update_barber_status',
      nonce:     six40Admin.nonce,
      barber_id: barberId,
      status:    status
    }, function (res) {
      if (res.success) {
        ...
        $card.find('.six40-barber-status-label')
          .attr('class', 'six40-status six40-status--' + status + ' six40-barber-status-label')
          .text(labels[status]);
        $card.find('.six40-barber-btn').each(function () {
          var $b = $(this);
          $b.prop('disabled', false)
            .text(btnLabels[$b.data('status')])
            .toggleClass('active', $b.data('status') === status);
        });
        toast('Estado de barbero actualizado.', 'success');
      } else {
        toast('Error: ' + (res.data || 'No se pudo actualizar.'), 'error');
  ...
  var calendarEl = document.getElementById('six40-calendar');
  ...
  function initCalendar() {
    calInstance = new FullCalendar.Calendar(calendarEl, {
      ...
      eventClick: function (info) {
        ...
        var msg = 'Cliente: ' + info.event.title +
                  '\nLocal: ' + p.location +
                  '\nEstado: ' + p.status +
                  '\nEmail: ' + p.email;
        alert(msg);
  ...
  function fetchEvents(info, successCb, failureCb) {
    var location = $('#six40-calendar-location').val() || '';
    $.get(six40Admin.ajaxUrl, {
      action:   'six40_get_appointments_json',
      nonce:    six40Admin.nonce,
      location: location
    }, function (res) {
      if (res.success) {
  ...
  // Reload calendar on location filter change.
  $(document).on('change', '#six40-calendar-location', function () {
    if (calInstance) calInstance.refetchEvents();
  });

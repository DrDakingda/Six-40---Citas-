...
function six40_init() {
    ...
    // Public scripts & AJAX handlers always registered.
...
function six40_enqueue_public_assets() {
    ...
    wp_enqueue_style(
        'six40-booking',
        SIX40_PLUGIN_URL . 'public/css/booking.css',
        [],
        SIX40_VERSION
    );
    wp_enqueue_script(
        'six40-booking',
        SIX40_PLUGIN_URL . 'public/js/booking.js',
        [ 'jquery' ],
        SIX40_VERSION,
        true
    );
...
function six40_ajax_submit_booking() {
    ...
    // Fire & forget: Google Calendar + confirmation email.
    $calendar = new Six40_Google_Calendar();
    $calendar->create_event( $result );

    $mailer = new Six40_Email();
    $mailer->send_confirmation( $result );
